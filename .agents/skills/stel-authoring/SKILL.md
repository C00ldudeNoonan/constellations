---
name: stel-authoring
description: Author stel projects and declarative data pipelines for unstructured documents. Use when creating, configuring, explaining, or troubleshooting stel sources, extraction models, transforms, chunk/embed/search flows, ML models, profiles, tests, selectors, artifacts, or dbt handoff; especially for a new project or a user-facing pipeline change.
---

# stel Authoring

Author a small, declarative pipeline that matches the user's data, warehouse, and privacy boundary. Treat stel as a standalone, dbt-shaped CLI; do not present it as a dbt package or adapter.

## Ground the work in the installed contract

1. Inspect an existing project's `stel_project.yml`, `profiles.yml`, `sources/`, `models/`, and `transforms/` before changing it.
2. For a new project, start with `uv run stel init <name> --template json|pdf|markdown|html` and modify its generated files rather than inventing a layout.
3. Run `uv run stel --help` and, when a source checkout is available, use its package README and closest example as the authority for feature syntax. Match the installed version; do not rely on generic dbt configuration or an assumed provider feature.
...
- Put `name`, `version`, `profile`, and project-controlled paths in `stel_project.yml`.
...
Declare `fields:` for a stable typed extraction payload, especially when a source can be empty. stel supplies lineage/provenance columns; do not restate them unless the current contract requests it. Layer parser-specific behavior under the backend's `options` block. Put domain logic in a downstream transform unless the source is an envelope whose raw payload must not cross the warehouse boundary; in that case use `extraction.post_extract` to return only the derived fields.
...
- Put repeated BigQuery layout policy in the active target's
  ...
  `connection`, and `external_volume`; stel derives a unique
...
```bash
uv run stel compile
uv run stel run --select <model-or-selector>
uv run stel show <model>
uv run stel test --select <model-or-selector>
```

Use `stel graph` to verify lineage and `target/manifest.json` / `target/run_results.json` to inspect compiled configuration, run counts, warnings, and safe failure details. Use selectors to limit work; `state:modified+` is appropriate for CI when a prior manifest is available.
