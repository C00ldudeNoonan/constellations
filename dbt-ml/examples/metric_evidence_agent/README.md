...
dbt MCP owns the governed metric query. dbt-ml MCP owns governed document
...
flowchart LR
    ...
    Q --> C["dbt-ml MCP: list/search/get/lineage"]
    M --> A["Metric: 4% → 7%"]
...
- all four dbt-ml context tools with entity and effective-date filters;
...
never included in tool arguments, output, or artifacts. The dbt-ml side and
