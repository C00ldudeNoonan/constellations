...
concept-to-concept edges), and a DAG (the downstream dbt manifest, or dbt-ml's
...
# dbt-ml DAG node kind -> a resource_type label for the plane.
_DBT_ML_KIND_RESOURCE = {"source": "source", "model": "model", "search_index": "exposure"}
...
def dag_plane_from_dbt_ml_manifest(manifest: dict[str, Any]) -> tuple[DagPlane, dict[str, str]]:
    """Plane from dbt-ml's own manifest. Returns the plane and a name→node-id map
    ...
    for entry in dag.get("nodes", []):
        ...
        nodes.append(DagNode(
            ...
            resource_type=_DBT_ML_KIND_RESOURCE.get(entry.get("kind", "model"), "model"),
        ))
    edges = []
...
def export_concept_cloud(
    ...
    *,
    ...
    if dbt_manifest is not None:
        ...
        dag_plane, id_by_name = dag_plane_from_dbt_ml_manifest(manifest)
