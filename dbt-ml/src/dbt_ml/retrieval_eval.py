    ...
    *,
    ...
    for row in golden_rows:
        ...
        request = SearchRequest(
            ...
            vector=_parse_vector(row.get("query_vector")),
            mode=SearchMode(row["mode"]) if row.get("mode") else mode,
            limit=limit,
            filters=_parse_filters(row.get("filters")),
        )
        policy_filters = _parse_filters(row.get("policy_filters"))
        try:
            hits = search(
                project_dir,
                request,
        ...
        relevant_ids = set(_parse_ids(row.get("relevant_ids")))
        graded = _parse_graded_relevance(row.get("graded_relevance"))
        per_query.append(
            evaluate_query(
                query_id,
                ranked_ids,
...
def _parse_graded_relevance(value: Any) -> dict[str, float] | None:
    parsed = _parse_json(value)
    if not parsed:
        return None
...
def _parse_filters(value: Any) -> tuple[SearchFilter, ...]:
    parsed = _parse_json(value)
    if not parsed:
        return ()
    return tuple(
