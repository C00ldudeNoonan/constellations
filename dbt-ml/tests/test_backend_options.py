...
def test_shipped_backends_expose_distinct_implementation_identities() -> None:
    ...
    assert all(identity.startswith("dbt-ml/") for identity in identities)
...
def test_compile_rejects_invalid_html_selector(tmp_path: Path) -> None:
    (tmp_path / "dbt_ml_project.yml").write_text("name: selector_contract\n")
...
def test_html_parser_error_only_advertises_available_parsers(
    ...
    monkeypatch.setattr(
        "dbt_ml.backends.options._available_html_parsers",
        lambda: ("html.parser",),
    )
