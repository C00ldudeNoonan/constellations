...
Some names dbt-ml uses are written into places that outlive a single run: a
...
import dbt_ml
from dbt_ml import _distribution, hashing, logging_setup
from dbt_ml import env as env_module
...
from dbt_ml.adapters.base import (
...
from dbt_ml.checks.schema import _failures_table_name
from dbt_ml.classic_ml.artifacts import ARTIFACT_RUNTIME_VERSION_KEY
from dbt_ml.config.identifiers import (
    DEFAULT_DUCKDB_FILENAME,
    DEFAULT_SCHEMA_NAME,
    RESERVED_PREFIX,
)
from dbt_ml.config.profile import WarehouseConfig
from dbt_ml.dbt_export import DBT_META_NAMESPACE
from dbt_ml.execution import extraction as extraction_module
from dbt_ml.manifest import RUN_RESULTS_VERSION_KEY
from dbt_ml.retrieval import coordination, lancedb

SRC_ROOT = pathlib.Path(dbt_ml.__file__).parent
...
_FROZEN_LITERALS: tuple[tuple[str, object, object, str], ...] = (
    ...
    ("env.PROFILES_DIR_ENV", env_module.PROFILES_DIR_ENV, "DBT_ML_PROFILES_DIR", "operator-set"),
    ("env.VERBOSE_ENV", env_module.VERBOSE_ENV, "DBT_ML_VERBOSE", "operator-set"),
    (
        "env.PROVIDER_DEBUG_ENV",
        env_module.PROVIDER_DEBUG_ENV,
        "DBT_ML_DEBUG_PROVIDER_ERRORS",
        "operator-set",
    ),
    (
        "env.MCP_PRINCIPAL_ID_ENV",
        env_module.MCP_PRINCIPAL_ID_ENV,
        "DBT_ML_MCP_PRINCIPAL_ID",
...
def test_verbose_logger_namespace_matches_the_package() -> None:
    ...
    assert logging_setup._ROOT_LOGGER == dbt_ml.__name__
...
def test_resolved_version_is_not_unknown() -> None:
    ...
    assert dbt_ml.__version__ != _distribution.UNKNOWN_VERSION
...
def test_type_identity_is_qualified_by_module() -> None:
    # Fingerprinting a value that carries a dbt-ml-defined pydantic model or
    ...
    # ever renamed.
    assert (
        hashing._type_identity(WarehouseConfig()) == "dbt_ml.config.profile.WarehouseConfig"
    )
...
def _env_literals() -> list[str]:
    """Bare `DBT_ML*` string literals passed to an environment read."""
    offenders: list[str] = []
    for path in _source_files():
        for node in ast.walk(ast.parse(path.read_text())):
            for argument in _env_read_arguments(node):
                if (
                    ...
                    and argument.value.startswith("DBT_ML")
                ):
...
def test_no_bare_env_var_literals() -> None:
    ...
    assert not offenders, (
        "Environment variables must be read through the constants in dbt_ml.env "
...
def test_env_scan_recognizes_every_form_the_code_could_regress_to() -> None:
    ...
    source = (
        "import os\n"
        'a = os.environ.get("DBT_ML_VERBOSE", "")\n'
        'b = os.environ["DBT_ML_VERBOSE"]\n'
        'c = os.getenv("DBT_ML_VERBOSE")\n'
        'd = read_env("DBT_ML_VERBOSE")\n'
    )
    matched = [
    ...
    assert matched.count("DBT_ML_VERBOSE") == 4
