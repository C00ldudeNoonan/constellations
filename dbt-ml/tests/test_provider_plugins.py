...
from dbt_ml.providers import registry as registry_module
from dbt_ml.providers.base import validate_profile_options_model
...
def test_entry_point_group_carries_contract_version() -> None:
    assert entry_point_group("inference") == (
        f"dbt_ml.inference_providers.v{PROVIDER_CONTRACT_VERSION}"
    )
...
def test_wrong_contract_version_reports_mismatch_not_missing(
    ...
    _install_plugin(
        ...
        group="dbt_ml.inference_providers.v2",
    )
    _discover_from(tmp_path / "site", monkeypatch)
...
def test_invalid_plugin_metadata_fails_registration(
    ...
    source = """
    from dbt_ml.providers import InferenceProvider, InferenceResult
...
def test_anthropic_truncated_response_keeps_billed_usage() -> None:
    from dbt_ml.providers.anthropic import _parse_response_safely, _with_billed_failure
    from dbt_ml.providers.base import InferenceRequest, ProviderResponseError
