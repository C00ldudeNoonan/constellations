...
stel treats hosted inference as an execution capability, not as a property of
...
`stel.providers` exposes frozen request and result envelopes:
...
- `BatchInferenceRequest` adds a stel-owned request ID. A
...
Importing `stel.providers` registers built-in providers. Separately packaged
...
entry-point groups, so the stock `stel` CLI can load them without a wrapper
import:

```toml
[project.entry-points."stel.inference_providers.v3"]
acme = "acme_stel.provider:AcmeInferenceProvider"

[project.entry-points."stel.embedding_providers.v3"]
acme = "acme_stel.provider:AcmeEmbeddingProvider"
```
...
- Loaded classes pass the same registration validation as built-ins
  (metadata, batch surface, constructor shape). `stel providers list` shows
...
distributions. It deliberately excludes the stel release and module source
...
the LLM backend implementation, so current stel package upgrades still
...
model, system-prompt fingerprint, provider implementation, and stel helper
...
combination of flags — see [ADR-0013](../adr/0013-a-debug-switch-owns-its-destination.md). The compatibility helper
...
provider layer: `stel.budget` defines strictly typed per-model
