...
administrative recovery (`stel serving`), and governed policy-prefilter
...
stel will represent a serving index as a distinct `search:` DAG resource and
...
- A collection with stale or failed governed publication state is unavailable
  through the stel query surface until a successful reconcile marks it ready.
...
full-text, and scalar indexes, while the stel resource represents the whole
      ...
      retrieval:
        ...
        stores:
          primary:
            ...
            path: target/lancedb
            # path: gs://econ-data-prod/stel/lancedb
...
Reserved physical fields use a versioned stel namespace. User mappings that
...
class CollectionMetadata(BaseModel):
    status: Literal["building", "ready", "failed"]
    ownership: Literal["stel", "external", "unknown"]
...
without the reserved stel ownership/contract metadata is external or unknown
...
of normal reconcile or `stel clean`; only a separately confirmed destructive
...
host stel's idempotency ledger. Stable whole-record upsert/delete semantics
...
stel does not automatically delete the old collection or state because it
may still serve another deployment. Remote cleanup must be an explicit,
destructive command in a later issue; `stel clean` remains local-artifact
cleanup only.
...
claim aborts before any further store I/O; `stel serving recover` refuses to
...
bypasses this readiness policy and is outside stel's authorization boundary;
...
#### What proves atomic activation (issue #355)

The proof is not store-side. The active generation is resolved through the
warehouse-owned serving ledger, so activation is a fenced warehouse row update
on the same machinery as `replace_state_scope` — the store is never asked to
swap anything and needs no alias primitive or conditional write.

What the store must prove is narrower, and is advertised as
`private_generation_build`: it can create a collection under a caller-chosen
physical name that receives no production queries, and drop one later. LanceDB
...
compute the scope that names the active generation. `physical_collection()`
therefore returns the unsuffixed, pre-#355 name when no generation is
requested, because that is where every already-published index still lives.
...
`stel eval` runs deterministic, CI-friendly retrieval tests over hand-labeled
...
`golden_set` is a `ref()` to an ordinary stel model of any kind (most
...
edges from `tests:`), so `stel run`/`build` always materialize the golden set
...
`stel eval [--select] [--exclude] [--json]` runs every `retrieval_tests` entry
on the selected search models (selector semantics match `stel test`) and
...
`fail` regardless. `stel eval` exits 1 if any test's status is `fail`,
matching `stel test`'s severity/exit-code convention; a `RetrievalEvalError`
...
LanceDB is not treated as stel's identity provider. Authorization requires
...
- #135 delivered the portable request/result/filter types, Python API,
  `stel search`, score normalization, and core RRF.
