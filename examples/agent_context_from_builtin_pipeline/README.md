...
uv run stel --project-dir examples/agent_context_from_builtin_pipeline run
```
...
`raw_research_notes` and `research_note_chunks` are ordinary stel
...
After `stel run`, `target/manifest.json` carries an `agent_context`
...
`list_context_models` uses (`src/stel/mcp_server/catalog.py`) will surface
it.
