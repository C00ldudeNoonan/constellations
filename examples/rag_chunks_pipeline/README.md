...
uv run stel --project-dir examples/rag_chunks_pipeline run
...
The portable `stel search` command and governed indexes with trusted
...
natively, so the machine that runs `stel build` and the machines that serve
...
embedding:
  ...
  provider_options:
    ...
    location: us-central1   # a region; embeddings on `global` cost ~10s on a
                            # cold connection, so a sparsely-queried served
                            # index pays it per query (issue #536)
...
```bash
uv run stel --project-dir examples/rag_chunks_pipeline eval
```
...
`stel eval` therefore exits `1` on this project by design (both
`retrieval_tests` entries run together — selection is per search model, like
`stel test`, not per named test). Inspect `target/retrieval_eval.json` for
