...
In embedded mode dbt — not stel — owns materialization: the dbt Python model
returns a frame and dbt runs the CREATE TABLE. So stel must *produce* the
...
stel's lifecycle/state/read machinery still works, but the three materialize
...
class CaptureAdapter(DuckDBAdapter):
    ...
    def __init__(
        ...
        # deleted on close; nothing stel-owned is persisted to the dbt database.
        self._scratch_dir = tempfile.TemporaryDirectory(prefix="stel_embed_")
    ...
    def materialize_incremental(
        ...
        *,
        ...
        # Embedded mode has no stel-side incremental state: dbt owns the table
        ...
        # stel-level change-detection fingerprint does not apply here.
        self._captured = df
        return df.height
