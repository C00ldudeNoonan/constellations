"""Library entry point for running one stel model from a dbt Python model.

`materialize(model, project_dir=..., session=...)` runs a single stel model's
...
so no stel-owned table or state is written — dbt owns the target.
"""
...
def materialize(
    ...
    *,
    ...
    """Run stel model `model` and return its output as a polars DataFrame.

    Parameters mirror what a dbt Python model can supply: `project_dir` points at
    the colocated stel project; `session` is the dbt-duckdb connection (reserved
    for the bidirectional `dbt_ref` path). `upstreams` maps each stel dependency
    name to the frame the dbt Python model read from `dbt.ref(...)` — transform
    models read their inputs from here instead of a stel warehouse.
    `target`/`profiles_dir` select the stel profile that carries backend/LLM
    configuration.
    """
    ...
    if model not in models_by_name:
        ...
        raise ValueError(
            f"Model '{model}' is not defined in stel project "
