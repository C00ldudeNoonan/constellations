"""Environment-variable names stel reads, and the single reader for them.

Every `STEL_*` name the tool itself resolves is declared here so the set is
enumerable and a rename is one diff rather than a grep. Operator-chosen
credential variables are deliberately absent: stel never names those, it
...
MCP_PRINCIPAL_ID_ENV = "STEL_MCP_PRINCIPAL_ID"
MCP_TENANT_ID_ENV = "STEL_MCP_TENANT_ID"
