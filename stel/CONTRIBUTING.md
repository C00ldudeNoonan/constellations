# Contributing to stel

stel is a small, focused Python project. Contributions welcome.
...
project — `src/stel`, `tests`, `examples`, `scripts`, and the focused static
...
7. Add an init template under `src/stel/templates/<backend>/` if a fresh-
...
5. Keep imports needed only by the hook lazy and document the required stel
...
in stel's installed backend registry.

## Adding a built-in Python transform

1. Put the module under the appropriate package, such as
   `src/stel/text/transforms/`, and expose `run(deps, ctx)` with one or two
...
1. Add a strict options model in `src/stel/text/linking.py` that subclasses
...
1. Add a strict Pydantic block to `src/stel/config/model.py`, wire it into
   `ModelConfig`, `_validate_single_kind`, `kind_block_count`, and the
   `ModelFile` missing-kind message.
2. Add an execution function in `src/stel/runner.py` and a dispatch branch in
...
3. Preflight the kind in `src/stel/compiler.py` (`_kind_label`, dependency and
   provider validation, warehouse capability requirements).
4. Fold its code identity into `src/stel/versioning.py` and surface it in
   `src/stel/manifest.py`, `src/stel/cli.py`, and the docs template.
...
1. Add the name to `SUPPORTED_TESTS` in `src/stel/checks/schema.py`.
...
1. Add the click subcommand in `src/stel/cli.py`.
...
Some strings stel uses are data, not code: warehouse table names and prefixes,
...
- stel is dbt-shaped but standalone. Its artifacts and test semantics must be
...
test: cover profile lookup with $STEL_PROFILES_DIR
