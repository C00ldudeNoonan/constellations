...
`agent_context/v1` is the warehouse contract between stel document pipelines,
...
The machine-readable source of truth is `stel.agent_context`. A warehouse
...
`entity_link_id`. `stel.agent_context.project_entity_link` bridges a matched
...
| Field | Logical type | Null | Meaning |
| --- | --- | --- | --- |
| `upstream_unique_id` | string | no | Upstream dbt or stel resource `unique_id`. |
...
version=1, digest_size=16)`. It prefixes `stel-canonical-fingerprint`, then
...
implementation in `stel.hashing` is normative.
...
For Python-backed transform models, stel validates contract columns,
...
declare `agent_context:` on the transform. `stel.agent_context.
...
and places contract metadata under `meta.stel.agent_context`; JSON and array
