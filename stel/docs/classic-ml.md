...
stel should treat classic text and document ML as a primary lane, not just a
...
`ml` extra (`pip install 'stel[ml]'`, which installs scikit-learn).
...
| `load_pretrained` | Apply a compatible stel-native artifact produced elsewhere, without fitting it. |
...
files are not accepted yet; pretrained artifacts must use the stel schema and
