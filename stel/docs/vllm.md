...
stel profile must match either the model passed to `vllm serve` or an explicit
...
Set `VLLM_API_KEY=local-vllm-token` in the process running stel. If the vLLM
...
stel does not submit vLLM's native batch endpoint, so `batch: true` is not
