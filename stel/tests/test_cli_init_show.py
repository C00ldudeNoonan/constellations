...
from stel.cli import _safe_console_text, cli
...
def test_init_with_pdf_template(tmp_path: Path) -> None:
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path) as cwd:
        ...
        assert "default_backend: pdf" in (target / "stel_project.yml").read_text()
...
def test_show_missing_db(tmp_path: Path, example_project_dir: Path) -> None:
    ...
    assert result.exit_code != 0
    assert "Run `stel run`" in result.output
