...
from stel.dbt_export import build_dbt_sources, write_dbt_sources
...
def test_custom_source_name(fresh_project: Path) -> None:
    payload = build_dbt_sources(fresh_project, source_name="my_stel")
    assert payload["sources"][0]["name"] == "my_stel"
