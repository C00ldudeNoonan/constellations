...
from stel.backends.base import ExtractionResult
...
def test_message_batch_uses_custom_api_key_env(
    ...
    monkeypatch.setenv("STEL_BATCH_KEY", secret)
...
def test_message_batch_missing_custom_key_fails_before_client_creation(
    ...
    monkeypatch.delenv("STEL_BATCH_KEY", raising=False)
    monkeypatch.setattr("anthropic.Anthropic", _FakeAnthropic)
    ...
    assert "STEL_BATCH_KEY" not in str(exc_info.value)
