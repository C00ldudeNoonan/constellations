...
from stel.cli import _usage_summary, cli
from stel.manifest import write_manifest, write_run_results
...
def test_batch_cost_uses_selected_provider_multiplier(
    ...
    # resolves the provider in stel.execution.extraction (issue #190).
    monkeypatch.setattr(
        "stel.execution.extraction.get_inference_provider",
...
def test_compile_warning_uses_configured_api_key_env(
    ...
    assert "STEL_ANTHROPIC_KEY" not in result.output
...
def test_provider_failure_does_not_leak_secret_or_prompt(
    ...
    profiles.write_text(
        profiles.read_text().replace(
            "api_key_env: ANTHROPIC_API_KEY",
            "api_key_env: STEL_FAILURE_KEY",
        )
    )
    secret = "provider-failure-secret"
    prompt_marker = "private-prompt-content"
    monkeypatch.setenv("STEL_FAILURE_KEY", secret)
...
def test_opt_in_provider_debug_logs_redacted_sdk_diagnostics(
    ...
    assert "stel.providers.anthropic" in caplog.text
