...
def test_incremental_merge_logs_safe_publication_telemetry(
    ...
    # processed, and DML-affected rows so an operator can match stel's own
    ...
    with caplog.at_level(logging.INFO, logger="stel.adapters.bigquery"):
        adapter.materialize_incremental("docs", df, key_col="document_id")
...
def test_incremental_merge_telemetry_reaches_active_reporter() -> None:
    ...
    # reporter and the stel log handler is removed, so the telemetry must also
    # reach the reporter — otherwise `run -v`/`build -v` show no job id.
    from stel.progress import _NullReporter, set_reporter
...
def test_state_upsert_stages_and_merges_at_scale(
    ...
    monkeypatch.setattr("stel.adapters.bigquery._STATE_MERGE_INLINE_MAX", 3)
    monkeypatch.setattr("stel.adapters.bigquery._STATE_MERGE_LOAD_BATCH", 2)
    client = _FakeClient()
    adapter = _adapter(client)
...
def _adapter_with_iceberg_defaults(*, target_name: str = "prod") -> BigQueryAdapter:
    adapter = _adapter(
        warehouse_defaults={
            ...
            "external_volume": "gs://iceberg-bucket/stel",
...
def test_profile_warehouse_defaults_derive_target_dataset_model_uri() -> None:
    ...
    assert prod_options.storage_uri == "gs://iceberg-bucket/stel/prod/ds/filings"
    assert dev_options.storage_uri == "gs://iceberg-bucket/stel/dev/ds/filings"
    assert prod_options.connection == "proj.us.biglake"
    assert prod_options.labels == {"managed_by": "stel"}
...
def test_partition_expression_ddl_forms() -> None:
    ...
    def expr(**payload: Any) -> str:
        from stel.adapters.bigquery import BigQueryPartitionBy
...
def test_insert_overwrite_replaces_partitions_via_script() -> None:
    ...
    assert "DECLARE stel_partitions ARRAY<DATE>;" in script
    assert (
        "SET stel_partitions = ARRAY(SELECT DISTINCT `filing_date` FROM"
        in script
    )
    ...
    assert (
        ...
        "IN UNNEST(stel_partitions) THEN DELETE" in script
    )
...
def test_integration_warehouse_options_round_trip() -> None:
    ...
    try:
        with adapter:
            ...
            assert table.labels == {"managed-by": "stel-tests"}
...
_BQ_ICEBERG_CONNECTION = os.environ.get("STEL_BQ_TEST_CONNECTION")
_BQ_ICEBERG_STORAGE_URI = os.environ.get("STEL_BQ_TEST_STORAGE_URI")


@pytest.mark.skipif(
    not (_BQ_PROJECT and _BQ_ICEBERG_CONNECTION and _BQ_ICEBERG_STORAGE_URI),
    reason=(
        "set STEL_BQ_TEST_PROJECT, STEL_BQ_TEST_CONNECTION (a BigLake Cloud "
        "Resource connection), and STEL_BQ_TEST_STORAGE_URI (a gs:// prefix) to "
...
def test_default_scopes_match_dbt_bigquery() -> None:
    from stel.adapters.bigquery import DEFAULT_SCOPES
...
def test_parse_keyfile_json_forms() -> None:
    ...
    from stel.adapters.bigquery import parse_keyfile_json
...
def test_fenced_replace_maps_gate_errors_without_leaking() -> None:
    ...
    client = _FakeClient()
    client.query_results = [
        _MessageFailingJob("400 stel-stale-state-fence: publication reassigned")
    ]
    adapter = _adapter(client)
    ...
    client = _FakeClient()
    client.query_results = [
        _MessageFailingJob("400 stel-state-replace-invalid: duplicate keys")
    ]
    adapter = _adapter(client)
...
def _fixed_staging_uuid(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("stel.adapters.base.uuid4", lambda: uuid_module.UUID(int=0))
...
def test_delete_rows_batches_large_key_sets(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("stel.adapters.bigquery._KEY_REQUEST_BATCH", 2)
    client = _FakeClient()
    client.tables["proj.ds.docs"] = ["document_id"]
