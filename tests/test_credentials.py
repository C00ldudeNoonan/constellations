...
from stel.config.profile import LLMConfig, ProfileConfig, TargetConfig
from stel.credentials import (
    CredentialFreeUrl,
    CredentialReference,
...
def _stel_traceback_locals(error: BaseException) -> str:
    rendered: list[str] = []
...
def test_literal_name_parser_does_not_accept_env_var_syntax() -> None:
    ...
    assert "DISTINCTIVE_CREDENTIAL_NAME" not in _stel_traceback_locals(
        exc_info.value
    )
...
def test_credential_url_error_does_not_retain_user_information() -> None:
    ...
    assert sentinel not in _stel_traceback_locals(exc_info.value)
    assert exc_info.value.__context__ is None
