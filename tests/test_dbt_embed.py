...
def test_custom_source_name_used_in_schema(fresh_project: Path, tmp_path: Path) -> None:
    out = tmp_path / "gen"
    generate_dbt_models(fresh_project, out, source_name="my_stel")
...
def test_materialize_transform_with_injected_upstream(fresh_project: Path) -> None:
    ...
    # dbt.ref(...), and assert stel runs the transform and returns its output.
...
# --- dbt_ref source: the reverse dbt->stel direction (#177) -----------------


@pytest.fixture
def dbt_ref_project(tmp_path: Path) -> Path:
    """A minimal stel project with one transform whose input is a dbt-built
    table (`source: dbt_ref('vendor_dim')`)."""
    proj = tmp_path / "dbt_ref_proj"
    (proj / "models").mkdir(parents=True)
    (proj / "transforms").mkdir()
    (proj / "stel_project.yml").write_text(
        "name: refproj\nversion: '0.1.0'\nprofile: refproj\n", encoding="utf-8"
    )
...
def test_materialize_dbt_ref_transform_reads_injected_dbt_table(dbt_ref_project: Path) -> None:
    ...
    # have read from dbt.ref('vendor_dim') and assert stel transforms it.
...
def test_standalone_run_rejects_dbt_ref_models(dbt_ref_project: Path) -> None:
    from stel.execution import RunError
...
# --- mixing a dbt_ref source with depends_on stel models (#177 follow-up) --


@pytest.fixture
def dbt_ref_mixed_project(tmp_path: Path) -> Path:
    """Like `dbt_ref_project`, but `enriched_vendors` also `depends_on:` a
    stel model (`region_lookup`), proving a transform can combine a
    dbt-built input with stel inputs in the same model."""
