...
from stel.adapters import AdapterError, create_adapter, parse_warehouse_config
...
def test_connect_passes_md_uri_and_reveals_token_only_into_native_config(
    ...
    monkeypatch.setattr("stel.adapters.duckdb.duckdb.connect", fake_connect)
...
def test_motherduck_capability_audit_against_live_service() -> None:
    ...
    database = os.environ.get("STEL_MOTHERDUCK_DATABASE", "md:")
    cfg = parse_warehouse_config(
        {
            "type": "duckdb",
            "schema": "stel_ci_motherduck",
            "path": database,
            "token": "{{ env_var('MOTHERDUCK_TOKEN') }}",
        }
    )
