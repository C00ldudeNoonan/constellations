...
from stel.config import ConfigError, SourceConfig, load_project
from stel.config.model import MLArtifactConfig, MLConfig, ModelConfig
from stel.config.project import ProjectConfig
from stel.ml_contracts import MLContractError, validate_ml_contract
from stel.paths import resolve_within_project
from stel.runner import run_project
from stel.sources import LocalDocumentSource, SourceError
...
def test_clean_removes_only_known_artifacts_and_preserves_warehouse(
    ...
    unknown.write_text("not owned by stel")

    runner = CliRunner()
    result = runner.invoke(cli, ["--project-dir", str(dst), "clean"])
    assert result.exit_code == 0, result.output
    assert warehouse.exists()
    assert unknown.read_text() == "not owned by stel"
