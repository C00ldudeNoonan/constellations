...
from stel.adapters import AdapterError, StateScope, create_adapter
...
def test_env_var_lookup(
    ...
    monkeypatch.setenv("STEL_PROFILES_DIR", str(other_dir))
    project, _, _ = load_project(project_dir)
...
def test_llm_options_merged_from_profile(tmp_path: Path) -> None:
    ...
    assert "STEL_ANTHROPIC_KEY" not in repr(reference)
    assert "STEL_ANTHROPIC_KEY" not in repr(options)
...
def test_profile_selected_provider_enforces_native_batch_capability(
    ...
    monkeypatch.setattr("stel.profile.get_inference_provider", lambda _name: provider)
    _write_project(tmp_path, profile="test_proj")
    _write_profiles(
        tmp_path,
        targets={
...
def test_provider_initialization_failure_is_a_profile_error(
    ...
    from stel.providers import ProviderConfigurationError

    def fail_provider(_name: str) -> None:
        raise ProviderConfigurationError("provider initialization failed safely")

    monkeypatch.setattr("stel.profile.get_inference_provider", fail_provider)
    _write_project(tmp_path, profile="test_proj")
    _write_profiles(
        tmp_path,
        targets={
    ...
    _write_profiles_raw(
        ...
        "\n".join(
            [
                ...
                "      warehouse:",
                "        type: duckdb",
                "        path: \"./target/{{ env_var('STEL_DB_NAME') }}.duckdb\"",
                "        schema: \"{{ env_var('STEL_SCHEMA', 'fallback_schema') }}\"",
            ]
        )
        + "\n",
    )
    monkeypatch.setenv("STEL_DB_NAME", "from_env")
    monkeypatch.delenv("STEL_SCHEMA", raising=False)
    ...
    _write_profiles_raw(
        ...
        "\n".join(
            [
                ...
                "      warehouse:",
                "        type: duckdb",
                "        path: \"{{ env_var('STEL_NOT_SET_ANYWHERE') }}\"",
            ]
        )
        + "\n",
    )
    monkeypatch.delenv("STEL_NOT_SET_ANYWHERE", raising=False)
    project, _, _ = load_project(tmp_path)
    with pytest.raises(ProfileError, match="STEL_NOT_SET_ANYWHERE"):
        resolve_profile(project, tmp_path)
...
def test_api_key_env_rejects_secret_interpolation_without_leaking_value(
    ...
    monkeypatch.setenv("STEL_SECRET_KEY", secret)
    _write_profiles_raw(
        tmp_path,
        "\n".join(
            [
                ...
                "      llm:",
                '        api_key_env: "{{ env_var(\'STEL_SECRET_KEY\') }}"',
            ]
        )
        + "\n",
    )
...
def test_environment_selected_bigquery_type_protects_credentials_first(
    ...
    monkeypatch.setenv("STEL_WAREHOUSE_TYPE", "bigquery")
    monkeypatch.delenv(env_name, raising=False)
    _write_profiles_raw(
        tmp_path,
        _bigquery_profile(
            [f'token: "{{{{ env_var(\'{env_name}\') }}}}"']
        ).replace("type: bigquery", 'type: "{{ env_var(\'STEL_WAREHOUSE_TYPE\') }}"'),
    )
    project, _, _ = load_project(tmp_path)
